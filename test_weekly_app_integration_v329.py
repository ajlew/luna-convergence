...
def test_weekly_studio_stays_hidden_and_contains_production_controls():
    ...
    assert "Download all seven daily scripts" in APP_SOURCE
    assert "Download 1080 × 1920 background" in APP_SOURCE
    assert "Download all 12 sign cards copy" in APP_SOURCE
    assert 'key="weekly-studio-week-v332"' in APP_SOURCE
    studio_source = APP_SOURCE.split("def weekly_studio_page()", 1)[1].split(
        "# ---------------------------------------------------------------------------\n# Historical context layer", 1
    )[0]
    assert 'with st.expander(item["sign"], expanded=False)' in studio_source
    assert "weekly_social_card_copy(item, monday)" in studio_source
